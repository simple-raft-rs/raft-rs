...
fn main() {
    ...
    let peers = peers_rx.into_iter().enumerate().map(|(peer_id, rx): (NodeId, _)| (
        RaftNode::new(
            peer_id,
            (0..5).collect(),
...
impl Network {
    fn send(&self, from: NodeId, sendable: SendableRaftMessage<NodeId>) {
        let message = IncomingMessage { from, message: sendable.message };
        match sendable.dest {
            RaftMessageDestination::Broadcast => {
                println!("peer {} -> all: {}", from, message.message);
                self.peers_tx.iter().for_each(|peer_tx| drop(peer_tx.send(message.clone())));
            }
